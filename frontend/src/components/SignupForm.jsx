...
import { AlertCircle, ArrowRight, Eye, EyeOff, Loader2, Lock, Mail, Sparkles, User } from 'lucide-react'
      ...
      <div className="relative w-full rounded-[26.5px] sm:rounded-[30.5px] bg-gradient-to-br from-[#faf0e8]/95 via-[#f4e4d7]/95 to-[#ead3c3]/95 backdrop-blur-2xl p-7 sm:p-9 overflow-hidden shadow-[inset_0_1px_2px_rgba(255,255,255,0.9),inset_0_-1px_2px_rgba(180,105,70,0.15)]">
        ...
        <form onSubmit={handleSubmit} className="relative z-10 space-y-4 sm:space-y-4.5" noValidate>
          ...
          <div>
            ...
            {usernameError && (
              <p id="signup-username-error" className="text-xs text-rose-700 font-medium pt-1">
                {usernameError}
              </p>
            )}
          </div>
          ...
          <div>
            <label
              ...
              className="block text-[13px] font-semibold text-[#2b180f] tracking-tight mb-2"
            >
              Email <span className="text-[#8a6a57] font-normal text-xs">(optional)</span>
            </label>
            <div className="relative">
              <input
                id="signup-email"
                ...
                disabled={isLoading}
                autoComplete="email"
                className="w-full h-12 rounded-xl bg-white/80 hover:bg-white/95 focus:bg-white border border-[#d9a88f]/60 hover:border-[#cb8362] focus:border-[#d96524] focus:ring-2 focus:ring-[#d96524]/20 focus:shadow-[0_0_12px_rgba(217,101,36,0.2)] text-[#2e190e] placeholder:text-[#917565] text-[14.5px] pl-11 pr-4 outline-none transition-all duration-150 disabled:opacity-50 caret-[#d96524] shadow-xs"
              />
              <Mail
                className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#8a6a57] pointer-events-none"
                aria-hidden="true"
          ...
          <div>
            ...
            {passwordError && (
              <p id="signup-password-error" className="text-xs text-rose-700 font-medium pt-1">
                {passwordError}
              </p>
            )}
          </div>
          ...
          <div>
            <label
              ...
              className="block text-[13px] font-semibold text-[#2b180f] tracking-tight mb-2"
            >
              Confirm password <span className="text-[#d96524]">*</span>
            </label>
            <div className="relative">
              <input
            ...
            {confirmError && (
              <p id="signup-confirm-error" className="text-xs text-rose-700 font-medium pt-1">
                {confirmError}
              </p>
            )}
          </div>
            ...
            <div className="flex items-center justify-between text-xs sm:text-[13px] pt-1">
              ...
              {onCancel && (
                <button
                  type="button"
                  onClick={onCancel}
                  className="text-[#6e5546] hover:text-[#22130b] font-medium transition-colors cursor-pointer"
                >
                  Back to Analyzer
                </button>
              )}
